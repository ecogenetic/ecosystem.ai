...
<Callout type="warning" title="params is a string">
Do not send a nested JSON object in `params`. Encode the object as a string. The closed-loop feedback endpoint is `POST /response` (singular).
</Callout>
...
This is the return for the Spend Personality API when called with `"params": "{\"flatten_json\": true}"`:
```yaml
openapi: 3.0.0
info:
  title: Spend Personality API
