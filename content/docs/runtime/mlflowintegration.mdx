...
<Callout type="tip" title="Looking to train models in-platform?">
  This page is for **importing** models from an external MLFlow
...
<Callout type="tip" title="Note">
    Integration is currently supported from models trained using H2O where either the mojo is stored as an artifact in MLFlow or the **Runtime MCP sidecar** has access to an h2o server used by MLFlow
</Callout>
