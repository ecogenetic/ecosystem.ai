...
The returned embeddings are bulk-upserted into
`ecosystem_meta.two_tower_user_embeddings` and
`ecosystem_meta.two_tower_item_embeddings` using the same document contract as
H2O exports. Real-time scoring then uses the same `PrePredictTwoTower` and
