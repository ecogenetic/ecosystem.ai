...
Configurations are immutable references; deletion is a cascade that
removes the configuration row, the linked feature pipeline, the
feature set, all training runs, and any K8s deployments tied to the
project id (see [Access](/docs/modules/mlrun/access)).

![MLRun configuration form](/images/modules/mlrun/mlrun-console-configuration-form.png)
