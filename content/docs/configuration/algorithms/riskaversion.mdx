...
`algorithm="ecosystem_rewards"` on `create_online_learning` is the SDK document type. Scoring uses `randomisation`: `"approach": "behaviorAlgos"`, `"sub_approach": "riskAversion"`. A longer `processing_window` (e.g. 30 days) is recommended to produce stable variance estimates.
</Callout>
