...
<Callout type="info" title="Runtime implementation">
The dynamic engagement path uses **Bernoulli** Naive Bayes (`BernoulliNaiveBayes`), not a multinomial classifier. Numeric Dynamic Recommender Training Fields are binned before `offer___variable___value` keys; the algorithm never concatenates a composite `training_cell`.
</Callout>
...
`algorithm="bayesian_probabilistic"` on `create_online_learning` is the SDK document type. Scoring uses `randomisation.approach` **`naiveBayes`**.
</Callout>
