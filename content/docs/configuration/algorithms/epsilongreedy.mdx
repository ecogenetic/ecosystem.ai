...
`algorithm="ecosystem_rewards"` on `create_online_learning` is the SDK document type, not the scoring algorithm. Set `randomisation.approach` to `epsilonGreedy`. The `epsilon` parameter in `define_deployment_multi_armed_bandit` sets the exploration rate.
</Callout>
