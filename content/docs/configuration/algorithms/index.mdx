...
<Callout type="info" title="Large-catalog retrieval">
The [Two-Tower recommender](/docs/modules/two_tower) uses `predictor.model.type=similarity` and embedding cosine/dot scoring. It is **not** a Dynamic Engagement `randomisation.approach` — it bypasses H2O and dynamic bandit scoring.
</Callout>

<Callout type="warning" title="Python SDK document type vs scoring approach">
`create_online_learning(..., algorithm="ecosystem_rewards")` (or `q_learning` / `bayesian_probabilistic`) creates a Dynamic Engagement **document**. Scoring is selected by `randomisation.approach` / `sub_approach` — the **Config value** on each algorithm page — not by that SDK argument. Do not invent approach IDs.
</Callout>
