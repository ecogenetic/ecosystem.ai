...
#### Debug (optional)
Add top-level `"debug": "true"` on `/invocations` to receive a sibling `debug` object with pipeline errors, Mongo status, and sanitized params when scoring fails silently. Use for developer diagnostics only — omit in production traffic. See [Runtime integration guide](https://github.com/ecogenetic/ecosystem-runtime/blob/main/ECOSYSTEM_INTEGRATION_RUNTIME.md#debug-response-opt-in).
