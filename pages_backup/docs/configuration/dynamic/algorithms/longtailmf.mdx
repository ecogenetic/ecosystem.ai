...
- The model needs **substantial implicit interaction history** to learn meaningful factors.
- **Unseen users and items** receive **random** latent initializations (typically small Gaussian noise, e.g. $\mathcal{N}(0, 0.01^2)$), so scores cluster near zero with noise until enough events accumulate.
