...
**Requires co-occurrence data**:

- With **no joint presentations** or no acceptance counts, the graph is empty or degenerate; normalization over outgoing weights can **divide by zero**.
- Offers with $\text{count} = 0$ **receive no edges** and do not participate like well-supported nodes.

<Callout type="warning" title="Empty graph">
Personalized PageRank can **crash or fail** when the interaction graph is empty or ill-formed. Ensure you have **meaningful co-occurrence and acceptance data** before enabling this algorithm in production.
</Callout>
