...
**Excellent** among behavioral algorithms:

- Default $\alpha = 1.0$, $\beta = 1.0$, and $\text{exposure} = 0$ yield $\mathrm{Beta}(1,1)$ — effectively uniform sampling for the draw.
...
- Together, this makes Coverage-Aware Thompson the strongest cold-start choice when catalog fairness and tail coverage matter.
